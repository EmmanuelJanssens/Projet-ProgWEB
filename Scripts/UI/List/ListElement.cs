    ...
    public virtual void Init()
    {
        ...
        for (int i = 0; i < cmds.Length; i++)
        {
            ...
            if (cmds[i].name == "Up")
            {
                ...
                cmdUp.onClick.AddListener(delegate { manager.onEvent.MovedUp(ID); });
            }
            else if (cmds[i].name == "Down")
            {
                cmdDown = cmds[i];
                cmdDown.onClick.RemoveAllListeners();
                cmdDown.onClick.AddListener(delegate { manager.onEvent.MovedDown(ID); });
            }
            else if (cmds[i].name == "Remove")
            {
                cmdRemove = cmds[i];
                cmdRemove.onClick.RemoveAllListeners();
                cmdRemove.onClick.AddListener(delegate { manager.onEvent.Delete(ID); });
            }
