            ...
            for(int i = 0; i < toggles.Length; i++)
            {
                if(toggles[i].isOn)
                {
                    ...
                    else if (typeof(T) == typeof(CTree))
                    {
                        ...
                        toAdd.sprite = UI_Selection.PreviewSprite[i];
                        toAdd.ui_object = UI_Selection.LinkedBundles[UI_Selection.PreviewSprite[i].name] as GameObject;
                        toAdd.Name = UI_Selection.PreviewSprite[i].name;
